...
export class StatefulSignal<TInput> {
  ...
  constructor(subject?: Subject<TInput | void>) {
    ...
    this.$signalSubject = subject ?? new Subject<void>();
  }

  create<TOutput>(createObservable: (input?: TInput) => Observable<TOutput>): Signal<StatefulValue<TInput>> {
    ...
    const determineInnerObservable = (inputValue) => {
      return inputValue 
    ...
    return toSignal(
      this.$signalSubject.pipe(
        ...
        switchMap(inputValue => {
          determineInnerObservable(inputValue).pipe(
            take(1),
            map(outputValue => ({
              result: outputValue as T,
              error: null,
