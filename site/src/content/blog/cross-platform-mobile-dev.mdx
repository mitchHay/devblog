...
Ultimately the choice is yours, a framework is just a tool on our developer toolbelts used to achieve a set of user requirements. What **is** important is to ensure you choose the **right** tool. If you aren't using a lot of animation, but want some native features it doesn't really make sense to choose something like Flutter. Similarly if you need a lot of animation, it doesn't make a whole bunch of sense to try and achieve that with something that doesn't have it's own rendering engine. If in doubt, refer to the [cross-platform guide](#the-cross-platform-guide) and work from there. Remember, with great power comes great responsibility.
